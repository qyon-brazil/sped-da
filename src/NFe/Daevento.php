...
class Daevento extends DaCommon
{
    ...
    private function header($x, $y, $pag)
    {
        ...
        } else if ($this->tpEvento == '210200') {
            $texto = '(Confirmação da Operação)';
        } else if ($this->tpEvento == '210210') {
            $texto = '(Ciência da Operação)';
        } else if ($this->tpEvento == '210220') {
            $texto = '(Desconhecimento da Operação)';
        } else if ($this->tpEvento == '210240') {
            $texto = '(Operação nao Realizada)';
        }
        ...
        $texto = 'Criado em : ' . date('d/m/Y H:i:s', $this->toTimestamp($this->dhEvento));
        $this->pdf->textBox($x, $y + 20, $w2, 8, $texto, $aFont, 'T', 'L', 0, '');
        $texto = 'Prococolo: '
            . $this->nProt
            . ' - Registrado na SEFAZ em: '
            . date('d/m/Y H:i:s', $this->toTimestamp($this->dhRegEvento));
        ...
        if ($this->tpEvento == '110110') {
            ...
            $texto = 'De acordo com as determinações legais vigentes, '
                . 'vimos por meio desta comunicar-lhe que a Nota Fiscal, '
                . 'abaixo referenciada, tem o evento ' . $texto . ' vinculado a NFe.';
        }
    ...
    private function body($x, $y)
    {
        ...
        if ($this->tpEvento == '110110') {
            ...
            $texto = !empty($this->xJust) ? $this->xJust : 'Confirmação da Operação';
        } else if ($this->tpEvento == '210210') {
            $texto = !empty($this->xJust) ? $this->xJust : 'Ciência da Operação';
        } else if ($this->tpEvento == '210220') {
            $texto = !empty($this->xJust) ? $this->xJust : 'Desconhecimento da Operação';
        } else if ($this->tpEvento == '210240') {
            $texto = !empty($this->xJust) ? $this->xJust : 'Operação não Realizada';
        }
        $aFont = ['font' => $this->fontePadrao, 'size' => 12, 'style' => 'B'];
