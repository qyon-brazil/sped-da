...
trait TraitBlocoIII
{
    protected function blocoIII($y)
    {
        ...
        $matrix = [0.12, $this->descPercent, 0.08, 0.09, 0.156, 0.172];
        ...
        $texto = "COD";
        $x = $this->margem;
        $this->pdf->textBox($x, $y + 1, ($this->wPrint * $matrix[0]), 3, $texto, $aFont, 'T', 'L', false, '', true);

        $texto = "DESCRIÇÃO";
        $x1 = $x + ($this->wPrint * $matrix[0]);
        $this->pdf->textBox($x1, $y + 1, ($this->wPrint * $matrix[1]), 3, $texto, $aFont, 'T', 'L', false, '', true);

        $texto = "QTD";
        $x2 = $x1 + ($this->wPrint * $matrix[1]);
        $this->pdf->textBox($x2, $y + 1, ($this->wPrint * $matrix[2]), 3, $texto, $aFont, 'T', 'C', false, '', true);

        $texto = "UN";
        $x3 = $x2 + ($this->wPrint * $matrix[2]);
        $this->pdf->textBox($x3, $y + 1, ($this->wPrint * $matrix[3]), 3, $texto, $aFont, 'T', 'C', false, '', true);

        $texto = "VL UN R$";
        $x4 = $x3 + ($this->wPrint * $matrix[3]);
        $this->pdf->textBox($x4, $y + 1, ($this->wPrint * $matrix[4]), 3, $texto, $aFont, 'T', 'C', false, '', true);

        $texto = "VL TOT R$";
        $x5 = $x4 + ($this->wPrint * $matrix[4]);
        $y1 = $this->pdf->textBox($x5, $y + 1, ($this->wPrint * $matrix[5]), 3, $texto, $aFont, 'T', 'R', false, '', true);
