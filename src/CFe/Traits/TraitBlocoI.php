...
trait TraitBlocoI
{
    protected function blocoI()
    {
        ...
        $emitCEP = $this->formatField($emitCEP, "#####-###");
        $emitUF = $this->getTagValue($this->enderEmit, "UF") ?: "SP";
        ...
        $texto = "CNPJ: {$emitCnpj} I.E.: {$emitIE} I.M.: {$emitIM}";
        $y += $this->pdf->textBox($xRs + 2, $y, $wRs - 2, 3, $texto, $aFont, 'T', $alignH, false, '', true);
        $texto = "Endereço: " . $emitLgr . ", " . $emitNro;
        $y += $this->pdf->textBox($xRs + 2, $y, $wRs - 2, 3, $texto, $aFont, 'T', $alignH, false, '', true);
        $texto = "Bairro: " . $emitBairro;
        $y += $this->pdf->textBox($xRs + 2, $y, $wRs - 2, 3, $texto, $aFont, 'T', $alignH, false, '', true);
        $texto = "CEP: " . $emitCEP . " - " . $emitMun . " - " . $emitUF . ($emitFone ? "  Fone: " . $emitFone : "");
        $y += $this->pdf->textBox($xRs + 2, $y, $wRs - 2, 3, $texto, $aFont, 'T', $alignH, false, '', true);
